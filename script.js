...
function render() {
  ...
  <main dir="${arabic ? "rtl" : "ltr"}">
    ...
    <section class="audience"><div class="shell"><b>${t("REMOTE SUPPORT ACROSS","دعم عن بُعد في")}</b><div>${[["Main Contractors","المقاولين الرئيسيين"],["Consultants","الاستشاريين"],["Developers","المطورين"],["Subcontractors","مقاولي الباطن"],["Overloaded Technical Offices","المكاتب الفنية تحت ضغط"]].map(x=>`<span>${t(x[0],x[1])}</span>`).join("")}</div></div></section>
