...
const dc = [
  ...
  ["Aconex Administration","إدارة المشروعات عبر Aconex","Workspace setup, document registers, transmittals, workflows, permissions, revisions and submission tracking.","تأسيس مساحة العمل والسجلات والتراسمات ودورات العمل والصلاحيات والإصدارات ومتابعة التقديمات"],
  ["SharePoint & Microsoft 365","SharePoint وMicrosoft 365","Project sites, controlled folders, metadata, document libraries, permissions, approvals and automated workflows.","مواقع المشروعات والمجلدات المنضبطة والبيانات التعريفية ومكتبات المستندات والصلاحيات والاعتمادات ودورات العمل الآلية"],
  ["Managed Monthly Support","متابعة شهرية مُدارة","Submission checks, live logs, pending and overdue monitoring, weekly reporting and management dashboards.","مراجعة التقديمات وتحديث السجلات ومتابعة المعلق والمتأخر والتقارير الأسبوعية ولوحات متابعة الإدارة"],
...
const remoteMarketGroups = [
  ["Europe","أوروبا",[["United Kingdom","المملكة المتحدة"],["Germany","ألمانيا"],["France","فرنسا"],["Italy","إيطاليا"],["Spain","إسبانيا"],["Netherlands","هولندا"],["Belgium","بلجيكا"],["Switzerland","سويسرا"],["Austria","النمسا"],["Sweden","السويد"],["Norway","النرويج"],["Denmark","الدنمارك"],["Ireland","أيرلندا"],["Finland","فنلندا"],["Poland","بولندا"]]],
  ["North America","أمريكا الشمالية",[["United States","الولايات المتحدة"],["Canada","كندا"],["Mexico","المكسيك"]]],
  ["Middle East & Gulf","الشرق الأوسط ودول الخليج",[["Saudi Arabia","السعودية"],["United Arab Emirates","الإمارات"],["Qatar","قطر"],["Kuwait","الكويت"],["Bahrain","البحرين"],["Oman","عُمان"],["Jordan","الأردن"],["Egypt","مصر"],["Iraq","العراق"],["Lebanon","لبنان"],["Türkiye","تركيا"]]],
  ["Asia Pacific","آسيا والمحيط الهادئ",[["India","الهند"],["Singapore","سنغافورة"],["Malaysia","ماليزيا"],["Australia","أستراليا"],["New Zealand","نيوزيلندا"],["Japan","اليابان"],["South Korea","كوريا الجنوبية"],["China","الصين"],["Hong Kong","هونغ كونغ"],["Indonesia","إندونيسيا"]]],
  ["Africa","أفريقيا",[["South Africa","جنوب أفريقيا"],["Morocco","المغرب"],["Algeria","الجزائر"],["Tunisia","تونس"],["Kenya","كينيا"],["Nigeria","نيجيريا"],["Ghana","غانا"],["Rwanda","رواندا"]]],
  ["Latin America","أمريكا اللاتينية",[["Brazil","البرازيل"],["Argentina","الأرجنتين"],["Chile","تشيلي"],["Colombia","كولومبيا"]]]
];

function remoteMarketsMarkup() {
  return remoteMarketGroups.map(([en, ar, countries]) => `<span class="market-group"><strong>${t(en, ar)}</strong><small>${countries.map(country => t(country[0], country[1])).join(" · ")}</small></span>`).join("");
}

function countryOptions() {
  const groups = remoteMarketGroups.map(([en, ar, countries]) => `<optgroup label="${t(en, ar)}">${countries.map(country => `<option value="${country[0]}">${t(country[0], country[1])}</option>`).join("")}</optgroup>`).join("");
  return `${groups}<option value="Other">${t("Other country", "دولة أخرى")}</option>`;
}

function populateCountryOptions() {
  const select = document.querySelector('#quoteForm select[name="country"]');
  if (select) select.innerHTML = `<option value="">${t("Select country", "اختر الدولة")}</option>${countryOptions()}`;
}
...
function render() {
  ...
  <main dir="${arabic ? "rtl" : "ltr"}">
    ...
    <section id="top" class="hero shell"><div><p class="eyebrow">${t("GLOBAL REMOTE PROJECT SUPPORT · DOCUMENT CONTROL FIRST","دعم مشروعات عالمي عن بُعد · إدارة المستندات أولًا")}</p><h1>${t("Document Control at the center of your project delivery.","إدارة المستندات في قلب تنفيذ مشروعك")}</h1><p class="lead">${t("NASQ provides remote Document Control and Project Support for contractors, consultants, developers and engineering offices across Europe, North America, the Middle East and Gulf, Africa, Asia-Pacific and Latin America. Document Control is our core service, supported by Aconex, SharePoint, Microsoft 365 and practical digital workflows.","تقدم نَسَق خدمات إدارة المستندات ودعم المشروعات عن بُعد للمقاولين والاستشاريين والمطورين والمكاتب الهندسية في أوروبا وأمريكا الشمالية والشرق الأوسط ودول الخليج وأفريقيا وآسيا والمحيط الهادئ وأمريكا اللاتينية. إدارة المستندات هي خدمتنا الأساسية بدعم من Aconex وSharePoint وMicrosoft 365 ودورات العمل الرقمية العملية")}</p><div class="actions"><a class="primary" href="#service-lines">${t("Explore services","استكشف الخدمات")}</a><a class="secondary" href="#contact">${t("Request a scope review","اطلب مراجعة نطاق العمل")}</a></div><div class="quick-tools"><a href="tools/document-control-health-check/">${t("Free project assessment","تقييم مجاني للمشروع")} ↗</a><a href="tools/scope-estimator/">${t("Estimate your service scope","حدد نطاق خدمتك")} ↗</a></div><div class="trust"><span>${t("Senior team expertise exceeding 20 years","خبرات قيادية للفريق تتجاوز 20 عامًا")}</span><span>${t("Europe · North America · Middle East & Gulf · Africa · Asia-Pacific · Latin America","أوروبا · أمريكا الشمالية · الشرق الأوسط ودول الخليج · أفريقيا · آسيا والمحيط الهادئ · أمريكا اللاتينية")}</span><span>${t("NDA and controlled access available","اتفاقية سرية وصلاحيات وصول منضبطة")}</span></div></div><aside class="status-card"><p>${t("CHOOSE YOUR SERVICE","اختر خدمتك")}</p><a href="#document-control"><b>01</b><span>Document Control<small>${t("Independent managed service","خدمة مُدارة مستقلة")}</small></span></a><a href="#technical-office"><b>02</b><span>Technical Office<small>${t("Architectural & structural","معماري وإنشائي")}</small></span></a><a href="#integrated"><b>03</b><span>Integrated Package<small>${t("Optional combined delivery","عرض متكامل اختياري")}</small></span></a></aside></section>

    <section class="audience"><div class="shell"><b>${t("REMOTE SUPPORT ACROSS KEY WORLD REGIONS","دعم عن بُعد في مختلف مناطق العالم")}</b><div>${remoteMarketsMarkup()}</div></div></section>

    <section id="service-lines" class="section shell"><p class="eyebrow">${t("TWO INDEPENDENT SERVICE LINES","مساران خدميان مستقلان")}</p><h2>${t("Document Control is our core service. Add technical support when your project needs it.","إدارة المستندات هي خدمتنا الأساسية ويمكنك إضافة الدعم الفني عند احتياج مشروعك")}</h2><div class="pillars document-control-led"><article class="primary-pillar"><span>01 · CORE SERVICE</span><h3>NASQ Document Control</h3><p>${t("For clients who need project information organized, tracked, reported and handed over—regardless of who prepares the drawings.","للعملاء الذين يحتاجون تنظيم معلومات المشروع ومتابعتها وإعداد تقاريرها وتسليمها بغض النظر عن الجهة التي تعد اللوحات")}</p><a href="services/document-control/">${t("Explore Document Control service","استكشف خدمة إدارة المستندات")} →</a><a class="pillar-secondary" href="#document-control">${t("View service details","عرض تفاصيل الخدمة")} →</a><div class="tool-tags"><span>Aconex</span><span>SharePoint</span><span>Microsoft 365</span><span>Excel · Power BI</span></div></article><article><span>02</span><h3>NASQ Technical Office</h3><p>${t("For clients who need architectural or structural shop drawings, coordination, revisions or as-built packages—regardless of who controls the documents.","للعملاء الذين يحتاجون لوحات تنفيذية معمارية أو إنشائية أو تنسيقًا ومراجعات أو لوحات ما تم تنفيذه بغض النظر عن الجهة التي تدير المستندات")}</p><a href="services/technical-office/">${t("Explore Technical Office service","استكشف خدمة المكتب الفني")} →</a><a class="pillar-secondary" href="#technical-office">${t("View service details","عرض تفاصيل الخدمة")} →</a></article></div></section>

    <section id="document-control" class="section alt"><div class="shell"><p class="eyebrow">01 · NASQ DOCUMENT CONTROL</p><h2>${t("The information system behind a controlled project","نظام المعلومات الذي يحافظ على انضباط المشروع")}</h2><p class="section-intro">${t("We establish and operate the document workflows, registers, permissions, submissions, reports and archives that keep every project record current, traceable and available to the right person at the right time.","نؤسس وندير دورات العمل والسجلات والصلاحيات والتقديمات والتقارير والأرشيف التي تجعل كل مستند في المشروع محدثًا وقابلًا للتتبع ومتاحًا للشخص المناسب في الوقت المناسب")}</p><div class="grid four">${serviceCards(dc,"DC")}</div></div></section>

    <section class="section tool-stack"><div class="shell"><p class="eyebrow">${t("REMOTE SYSTEMS & TOOLS","الأنظمة والأدوات عن بُعد")}</p><h2>${t("Connected workflows for distributed project teams","دورات عمل مترابطة لفرق المشروعات عن بُعد")}</h2><p class="section-intro">${t("NASQ can help organize the project environment around the tools already used by your team, subject to the client’s access, permissions and approved workflow.","تساعد نَسَق في تنظيم بيئة المشروع حول الأدوات التي يستخدمها فريقك بالفعل وفق صلاحيات الدخول وإجراءات العمل المعتمدة لدى العميل")}</p><div class="tool-grid"><article><b>Aconex</b><p>${t("Workspaces, transmittals, workflows and revision tracking","مساحات العمل والتراسمات ودورات العمل وتتبع الإصدارات")}</p></article><article><b>SharePoint</b><p>${t("Sites, libraries, metadata and controlled access","المواقع والمكتبات والبيانات التعريفية والوصول المنضبط")}</p></article><article><b>OneDrive & Microsoft 365</b><p>${t("Cloud filing, collaboration and controlled sharing","الحفظ السحابي والتعاون والمشاركة المنضبطة")}</p></article><article><b>Excel & Power BI</b><p>${t("Registers, dashboards, KPIs and management reports","السجلات ولوحات البيانات ومؤشرات الأداء وتقارير الإدارة")}</p></article><article><b>Microsoft Teams</b><p>${t("Project coordination, meetings and action follow-up","تنسيق المشروع والاجتماعات ومتابعة الإجراءات")}</p></article><article><b>Outlook & Email Workflows</b><p>${t("Professional correspondence and searchable project records","المراسلات المهنية وسجلات المشروع القابلة للبحث")}</p></article><article><b>EDMS / CDE Platforms</b><p>${t("Structured document environments aligned with project needs","بيئات المستندات المنظمة المناسبة لاحتياجات المشروع")}</p></article><article><b>Remote Handover</b><p>${t("Closeout indexes, archives and controlled delivery packages","فهارس التسليم والأرشيف وحزم التسليم المنضبطة")}</p></article></div></div></section>
    ...
    <section id="integrated" class="integrated"><div class="shell"><div><p class="eyebrow">03 · ${t("OPTIONAL INTEGRATED PACKAGE","العرض المتكامل الاختياري")}</p><h2>${t("From drawing preparation to approval-ready archive","من إعداد اللوحة إلى أرشيف منظم وجاهز للاعتماد")}</h2><p>${t("One coordinated team can prepare, review, code, submit, revise, track and archive your shop drawings. Each service remains available separately.","يمكن لفريق واحد إعداد اللوحات ومراجعتها وتكويدها وتقديمها وتعديلها ومتابعتها وأرشفتها مع بقاء كل خدمة متاحة بصورة مستقلة")}</p><a class="primary" href="#contact" data-service="Document Control & Technical Office">${t("Request integrated support","اطلب الدعم المتكامل")}</a><div class="integrated-resource"><span class="resource-kicker">${t("FREE PROJECT TOOL","أداة مجانية للمشروعات")}</span><h3>${t("Arabic Procurement Register Template","قالب سجل المشتريات العربي")}</h3><p>${t("A practical Excel template for tracking procurement items, suppliers, prices, delays and project documents.","قالب Excel عملي لمتابعة بنود المشتريات والموردين والأسعار والتأخيرات والمستندات المرتبطة بالمشروع.")}</p><div class="resource-features"><span>${t("Dashboard","لوحة بيانات")}</span><span>${t("Search panel","لوحة بحث")}</span><span>${t("Protected formulas","معادلات محمية")}</span></div><div class="resource-actions"><a class="primary" href="https://eslam-reda-dc.github.io/nasq-project-solutions/resources/procurement-register/downloads/NASQ%20Procurement%20Register%20Template%20AR.xlsx" target="_blank" rel="noopener" download data-resource="procurement-register">${t("Download free Excel template","تحميل قالب Excel مجانًا")} ↗</a><a class="resource-link" href="https://github.com/eslam-reda-dc/nasq-project-solutions/tree/main/resources/procurement-register" target="_blank" rel="noopener">${t("View guide and previews","عرض الدليل والمعاينات")} ↗</a></div></div></div><div class="cycle">${[["01","Review Inputs","مراجعة المدخلات"],["02","Prepare Drawings","إعداد اللوحات"],["03","Internal QA","المراجعة الداخلية"],["04","Code & Submit","التكويد والتقديم"],["05","Track Comments","متابعة الملاحظات"],["06","Revise & Archive","التعديل والأرشفة"]].map(x=>`<div><b>${x[0]}</b><span>${t(x[1],x[2])}</span></div>`).join("")}</div></div></section>
    ...
    <section id="founder" class="section shell founder-section">
      <div class="founder-card">
        ...
        <div class="founder-content">
          ...
          <p>${t("Construction and real estate documentation professional supporting structured document workflows, project coding, reporting, electronic archives and remote project delivery for international teams, with hands-on experience across Egypt and Saudi Arabia.","متخصص في إدارة مستندات مشروعات الإنشاءات والتطوير العقاري، وتأسيس الدورات المستندية وأنظمة التكويد والتقارير والأرشفة الإلكترونية ودعم فرق المشروعات عن بُعد دوليًا، مع خبرة عملية في مصر والسعودية.")}</p>
          <div class="founder-metrics"><span>${t("3,500+ document references","أكثر من 3,500 مرجع مستندي")}</span><span>${t("160+ project reports","أكثر من 160 تقريرًا")}</span><span>${t("International remote delivery","دعم دولي عن بُعد")}</span></div>
 ...
 <article><h3>${t("Interactive project tools","أدوات تقييم المشروعات")}</h3><a href="tools/document-control-health-check/">${t("Document control health check","تقييم إدارة المستندات")} ↗</a><a href="tools/scope-estimator/">${t("Project support scope estimator","تقدير نطاق دعم المشروع")} ↗</a><a href="tools/dashboard-demo/">${t("Interactive dashboard demo","نموذج لوحة متابعة تفاعلية")} ↗</a><a href="https://eslam-reda-dc.github.io/nasq-project-solutions/resources/procurement-register/downloads/NASQ%20Procurement%20Register%20Template%20AR.xlsx" target="_blank" rel="noopener" download>${t("Free Arabic procurement register","قالب سجل المشتريات العربي المجاني")} ↗</a><a href="cases/">${t("Professional case studies","دراسات الحالة المهنية")} ↗</a></article>
...
<a class="floating-whatsapp" href="https://wa.me/201091131666?text=Hello%20NASQ%2C%20I%20would%20like%20to%20discuss%20project%20support." target="_blank" rel="noopener" aria-label="WhatsApp">${t("WhatsApp","واتساب")}</a>
